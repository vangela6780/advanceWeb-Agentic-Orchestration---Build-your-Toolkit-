...
import { CommandNotFoundError } from '../infrastructure/errorHandler';
 ...
 */
export class StandardCLIEngine implements CLIEngine {
  constructor(
  ...
  async execute(args: string[]): Promise<CommandResult> {
    ...
    try {
      ...
      if (context.arguments.length === 0) {
        throw new CommandNotFoundError('No command specified');
      }
      ...
      const result = this.errorHandler.handle(error as Error);
  ...
  async executeCommand(commandName: string, context: CLIContext): Promise<CommandResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const result = this.errorHandler.handle(error as Error, context);
      result.executionTimeMs = Date.now() - startTime;
